            ...
            <div class="box" :style="{backgroundImage:'url('+musicImg+')'}" :class="{box_active:disActive}" @dblclick="DisList">
                ...
                <div class="control_box">
                    ...
                    <div class="myprogress">
                        <div class="myprogress_c" :style="{width:currentProgress}">
                            <div class="myprogress_circle">
                                <div class="myprogress_circle_c"></div>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        Player(){
            ...
            //进度条控制
            $('.myprogress').on('mousedown',(ev)=>{
                let e=ev||event;
                let pro=((e.clientX-$('.myprogress').offset().left)/$('.myprogress').width())
                clearInterval(playerTimer);
                this.currentProgress=`${pro*100}%`
                $(document).on('mousemove',(ev)=>{
                    let e=ev||event;
                    pro=((e.clientX-$('.myprogress').offset().left)/$('.myprogress').width())
                    this.currentProgress=`${pro*100}%`
                })
                $(document).on('mouseup',()=>{
